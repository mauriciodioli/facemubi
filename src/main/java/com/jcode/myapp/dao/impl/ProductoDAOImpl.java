...
public class ProductoDAOImpl implements ProductoDAO {
            ...
            while (rs.next()) {
                ...
                if (rs.getInt("COUNT") > 0) {
                    ...
                    while (rs.next()) {
                        ...
                        producto.setNombreImagenBoton(rs.getString("NOMBREIMAGENBOTON"));
                        producto.setArchivoimg2(rs.getBytes("ARCHIVOIMAGEN"));
                        producto.setCategoria(new Categoria(rs.getInt("IDCATEGORIA")));
                        producto.setIp(producto.getIp());
                        list.add(producto);
                    }
                }
            }
    ...
    public BEAN_CRUD add(Producto obj, HashMap<String, Object> parameters) throws SQLException {
        ...
        try (Connection conn = this.pool.getConnection();
            ...
            while (rs.next()) {
                if (rs.getInt("COUNT") == 0) {
                    pst = conn.prepareStatement("INSERT INTO PRODUCTO(NOMBRE,PRECIO,STOCK,STOCK_MIN,STOCK_MAX,IDCATEGORIA,IDEMPRESA) VALUES(?,?,?,?,?,?,?)");
                    pst.setString(1, obj.getNombre());
            ...
            while (rs.next()) {
                if (rs.getInt("COUNT") == 0) {
                    pst = conn.prepareStatement("UPDATE PRODUCTO SET NOMBRE = ?, PRECIO = ?, STOCK = ?, STOCK_MIN = ?, "
                            + "STOCK_MAX = ?, NOMBREIMAGENBOTON = ?, IDCATEGORIA = ?, ARCHIVOIMAGEN = ? WHERE IDPRODUCTO = ?");
                     
                    pst.setString(1, obj.getNombre());//LOG.info("DAOIMPLLLLLL getNombre "+obj.getNombre());
                    pst.setDouble(2, obj.getPrecio());// LOG.info("DAOIMPLLLLL getPrecio "+obj.getPrecio());
                    pst.setInt(3, obj.getStock());//LOG.info("DAOIMPLLLLL  getStock "+obj.getStock());
                    pst.setInt(4, obj.getStock_min());//LOG.info("DAOIMPLLLLL getStock_min "+obj.getStock_min());
                    pst.setInt(5, obj.getStock_max());//LOG.info("DAOIMPLLLLL getStock_max "+obj.getStock_max());
                    pst.setString(6, obj.getNombreImagenBoton());//LOG.info("DAOIMPLLLLL getNombreImagenBoton "+obj.getNombreImagenBoton());
                    pst.setInt(7, obj.getCategoria().getIdcategoria());//LOG.info("DAOIMPLLLLL getIdcategoria "+obj.getCategoria().getIdcategoria());
                    pst.setBlob(8, obj.getArchivoimg());
                    pst.setInt(9, obj.getIdproducto());//LOG.info("DAOIMPLLLLL getIdproducto "+obj.getIdproducto());
